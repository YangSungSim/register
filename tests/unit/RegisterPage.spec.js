...
describe('RegisterPage.vue', () => {
  ...
  beforeEach(() => { // 로컬 vue 인스턴스와 router 인스턴스를 mount 함수에 제공해 wrapper 를 생성
    wrapper = mount(RegisterPage, {
      localVue,
      router   //로그인 페이지로 리다이렉션이 발생했는지 확인 하는 데 필요
    })
  ...
  it('should register when it is a new user', async () => {
    expect.assertions(2)
    const stub = jest.fn()
    wrapper.vm.$router.push = stub // 리다이렉트가 발생했는지 확인
    wrapper.vm.form.username = 'sunny'
    wrapper.vm.form.emailAddress = 'sunny@taskagile.com'
    wrapper.vm.form.password = 'JestRocks!'
    wrapper.vm.submitForm() // 데이터를 넣어서 폼을 제출
    expect(registerSpy).toBeCalled()
    await wrapper.vm.$nextTick() // 에러메세지가 보이는지 검증
  ...
  it('should fail it is not a new user', async () => {
    ...
    wrapper.vm.form.password = 'JestRocks!'
    expect(wrapper.find('.failed').isVisible()).toBe(false) // 메소스가 호출되기 전에 에러메세지가 보이는지 검증
